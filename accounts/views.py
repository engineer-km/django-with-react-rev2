...
def profile_edit(request):
    ...
    return render(
        request, "accounts/profile_edit_form.html", 
        { "form": form, }
    )
...
def user_follow(request, username):
    ...
    messages.success(request, "{} 님을 팔로우 했습니다.".format(follow_user))
...
def user_unfollow(request, username):
    ...
    messages.success(request, "{} 님을 언팔 했습니다.".format(unfollow_user))
    redirect_url = request.META.get("HTTP_REFERER", "root")  # HTTP_REFERER: 요청온 주소 알아내기. 없으면 'root' 로 
    return redirect(redirect_url)